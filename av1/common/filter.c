...
sub_pel_filters_4[SUBPEL_SHIFTS]) = {
	...
	// 16, 4/2, 0.625, 6, manually adjusted
	// { 0, 0,   0, 128,  0,   0,   0, 0 },{ 0, 0, -4,  126,   8,   -2,   0, 0 },
	// { 0, 0,  -8, 122, 18,  -4,   0, 0 },{ 0, 0,-10,  116,  28,   -6,   0, 0 },
	// { 0, 0, -12, 110, 38,  -8,   0, 0 },{ 0, 0,-12,  102,  48,  -10,   0, 0 },
	// { 0, 0, -14,  94, 58, -10,   0, 0 },{ 0, 0,-14,   86,  66,  -10,   0, 0 },
	// { 0, 0, -12,  76, 76, -12,   0, 0 },{ 0, 0,-12,   66,  86,  -12,   0, 0 },
	// { 0, 0, -10,  58, 94, -14,   0, 0 },{ 0, 0,-10,   48, 102,  -12,   0, 0 },
	// { 0, 0,  -8,  38,110, -12,  0, 0 }, { 0, 0, -6,  28,   116, -10,   0, 0 },
	// { 0, 0,  -4,  18,122,  -8,   0, 0 },{ 0, 0, -2,   8,   124,  -4,   0, 0 }
};
DECLARE_ALIGNED(256, static const InterpKernel,
...
InterpFilterParams av1_get_interp_filter_params_with_block_size(
	...
	if (w == 2 || h == 2) {
		...
		else if (interp_filter == EIGHTTAP_SMOOTH) {
			return av1_interp_4tap[1];
		}
		else if (interp_filter == BILINEAR) {
			av1_interp_filter_params_list[BILINEAR];
		}
	}
	else if (w == 4 || h == 4) {
